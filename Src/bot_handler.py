...
def select_units(units):
    ...
    for new_unit in units:
        try:
            cv2.imwrite('units/' + new_unit,
                        cv2.imread('all_units/' + new_unit))
...
def bot_loop(bot, info_event):
    ...
    while (not bot.bot_stop):
        ...
        if output[1] == 'fighting':
            ...
            bot.getScreen()
            info = ''
            money = re.sub('\D', '', tesseract.do_ocr(
                bot.screenRGB[20:70, 360:535]))
            if _time == 0:
                _time = time.time()
                info = f'Start money: {money}'
            else:
                try:
                    new_time = time.time()
                    money_rate = round((int(money)-int(prev_money)) /
                                       ((new_time-_time)/3600), 2)
                    info = f'Money: {money} Money/hour: {money_rate}'
                    prev_money = money
                    _time = new_time
                except Exception as e:
                    print(e)
            ...
            try:
                asyncio.run(telegram_bot.send_image(path='money.png',
                                                    text=info, uid=config['telegram_chat_id'], token=config['telegram_token']))
            except Exception as e:
                print(e)
...
def check_scrcpy(logger):
    if os.path.exists('/usr/local/bin/scrcpy'):
        return True
    else:
        logger.info('scrcpy is not installed')
        # Download
        download(
            'https://github.com/Genymobile/scrcpy/releases/download/v1.24/scrcpy-win64-v1.24.zip', 'scrcpy.zip')
