...
class RR_bot:
    ...
    def update_units(self):
        self.selected_units = self.config['bot']['units'].replace(
            ' ', '').split(',')
        ...
        if not bot_handler.select_units([unit + '.png' for unit in self.selected_units]):
            valid_units = ' '.join(os.listdir("all_units")).replace(
                '.png', '').split(' ')
            self.logger.info(
    ...
    def start_bot(self):
        ...
        try:
            asyncio.run(telegram_bot.send_message(text='Bot started!',
                                                  uid=config['telegram_chat_id'], token=config['telegram_token']))
        except Exception as e:
            print(e)

        self.bot_instance = bot_handler.start_bot_class(self.logger)
        os.system("cat Src/startup_message.txt")
        self.update_units()
    ...
    def update_text(self, i, combat, output, grid_df, unit_series, merge_series, info):
        ...
        if grid_df is not None:
            grid_df['unit'] = grid_df['unit'].apply(
                lambda x: x.replace('.png', ''))
            grid_df['unit'] = grid_df['unit'].apply(
                lambda x: x.replace('empty', '-'))
            num_demons = str(
                grid_df[grid_df['unit'] == 'demon_hunter']['rank'].sum())
        ...
        if merge_series is not None:
            ...
            write_to_widget(self.root, self.merge_dump,
...
def create_options(frame1, config):
    ...
    # General options
    label = Label(frame1, text="Options", justify=LEFT).grid(
    ...
    # Mana level targets
    mana_label = Label(frame1, text="Mana Level Targets",
                       justify=LEFT).grid(row=2, column=0, sticky=W)
    stored_values = np.fromstring(
        config['bot']['mana_level'], dtype=int, sep=',')
    mana_vars = [IntVar(value=int(i in stored_values)) for i in range(1, 6)]
    mana_buttons = [
        Checkbutton(
            frame1, text=f'Card {i+1}', variable=mana_vars[i], justify=LEFT).grid(row=2, column=i + 1)
        for i in range(5)
    ]
    # Dungeon Floor
    floor_label = Label(frame1, text="Dungeon Floor",
